...
namespace VideoGameStore.Tests
{
    public class FeedbackControllerTests
    {
        ...
        [Fact]
        public void CreateFeedback_ReturnsInternalServerError_OnException()
        {
            // Arrange
            int productId = 1;
            var feedback = new Feedback { text = "Good game!", rating = 5.0f };
            ...
            _contextMock.Setup(context => context.CreateFeedbackForProduct(productId, feedback, "testUser"))
                        .Throws(new Exception("Database error"));

            // Act
            var result = _controller.CreateFeedback(productId, request) as ObjectResult;
        ...
        [Fact]
        public void ReportFeedback_ReturnsInternalServerError_OnException()
        {
            // Arrange
            int feedbackId = 1;
            _contextMock.Setup(context => context.ReportFeedback(feedbackId))
                        .Throws(new Exception("Database error"));

            // Act
            var result = _controller.ReportFeedback(feedbackId) as ObjectResult;
        ...
        [Fact]
        public void RateFeedback_ReturnsInternalServerError_OnException()
        {
            // Arrange
            int feedbackId = 1;
            int newRating = 4;
            _contextMock.Setup(context => context.RateFeedback(feedbackId, newRating))
                        .Throws(new Exception("Database error"));

            // Act
            var result = _controller.RateFeedback(feedbackId, newRating) as ObjectResult;
